...
compute_designs(int *strips_sums[], int cur_seqno, int depth)
{
    ...
    if (cur_seqno == -1)
    {
        ...
        for (int i = 0; i < seqno; i++)
            sum += compute_designs(strips_sums, i, 1);
    }
    else
    {
        ...
        for (int i = 0; i < seqno; i++)
        {
            if (!share_inner_edge(strips_sums, i, cur_seqno))
                sum += compute_designs(strips_sums, i, depth + 1);
        }
    }
...
main()
{
    ...
    /* TODO add a table where compatible seqnos can be seen
     ...
     */
    int n_designs = compute_designs(strips_sums, -1, 0);
